...
const ANIM_ORDER = ["os", "machine", "difficulty", "technique", "tag", "tool", "platform"];
...
export default function Graph() {
  ...
  const filterGroups = graphData
    ? [
        { id: "platform",   label: "Plataforma",  values: [...new Set(graphData.nodes.filter((n) => n.type === "platform").map((n) => n.label))] },
        { id: "os",         label: "Sistema OS",  values: [...new Set(graphData.nodes.filter((n) => n.type === "os").map((n) => n.label))] },
        { id: "difficulty", label: "Dificultad",  values: [...new Set(graphData.nodes.filter((n) => n.type === "difficulty").map((n) => n.label))] },
        { id: "technique",  label: "Técnica",     values: [...new Set(graphData.nodes.filter((n) => n.type === "technique").map((n) => n.label))] },
        { id: "tool",       label: "Herramienta", values: [...new Set(graphData.nodes.filter((n) => n.type === "tool").map((n) => n.label))] },
        { id: "tag",        label: "Tag",         values: [...new Set(graphData.nodes.filter((n) => n.type === "tag").map((n) => n.label))] },
      ]
    : [];

  function nodeMatchesFilter(node, filters) {
    if (filters.type === "all")        return true;
    if (filters.type === "platform")   return node.platform === filters.value || node.label === filters.value;
    if (filters.type === "os")         return node.os === filters.value || node.label === filters.value;
    if (filters.type === "difficulty") return node.difficulty === filters.value || node.label === filters.value;
    if (filters.type === "technique")  return (node.type === "technique" && node.label === filters.value) || node.type === "machine";
    if (filters.type === "tool")       return (node.type === "tool" && node.label === filters.value) || node.type === "machine";
    if (filters.type === "tag")        return (node.type === "tag" && node.label === filters.value) || node.type === "machine";
    return false;
  }
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const H = Math.max(500, Math.min(1000, graphData.nodes.length * 2.8));
  ...
  const runAnimation = useCallback(async () => {
    ...
    for (const groupType of ANIM_ORDER) {
      ...
      for (const l of groupLinks) {
        link.filter((d) => d === l).transition().duration(200)
          .attr("stroke-opacity", l.rel === "related" ? 0.07 : 0.18);
        await delay(groupLinks.length > 100 ? 4 : 15);
      }
      await delay(250);
    }
    setAnimating(false);
  ...
  const totalLinks = graphData?.links?.filter((l) => l.source !== l.target).length || 0;
  ...
  const FilterList = () => (
    <>
      <button
        ...
        style={{
          width: "100%", fontFamily: "monospace", fontSize: "0.7rem", padding: "4px 10px",
          cursor: "pointer",
      ...
      {filterGroups.map((group) => (
        <div key={group.id} style={{ marginBottom: "0.4rem" }}>
          <button
            ...
            style={{
              width: "100%", fontFamily: "monospace", fontSize: "0.65rem", padding: "4px 10px",
              cursor: "pointer", background: "transparent", border: "1px solid #1a3a4a",
              color: gColors[group.id] || "#4a6a7a", textAlign: "left",
              display: "flex", justifyContent: "space-between", alignItems: "center", letterSpacing: "1px",
            }}
            onMouseOver={(e) => (e.currentTarget.style.borderColor = gColors[group.id] || "#4a6a7a")}
            onMouseOut={(e)  => (e.currentTarget.style.borderColor = "#1a3a4a")}
          >
            <span>{group.label}</span>
            <span style={{
              fontSize: "0.6rem", display: "inline-block", transition: "transform 0.2s",
              transform: openSection === group.id ? "rotate(90deg)" : "rotate(0deg)",
            }}>▶</span>
          </button>
          {openSection === group.id && (
            <div style={{ background: "#060d14", border: "1px solid #1a3a4a", borderTop: "none" }}>
              {group.values.map((val) => {
                const isActive  = activeFilters.type === group.id && activeFilters.value === val;
                const nodeForVal = graphData.nodes.find((n) => n.label === val && n.type === group.id);
                const valColor  = nodeForVal?.color || subColors[val] || gColors[group.id] || "#4a6a7a";
                return (
                  <button
                    key={val}
                    onClick={() => setFilter(group.id, val)}
                    style={{
                      width: "100%", fontFamily: "monospace", fontSize: "0.65rem", padding: "3px 14px",
                      cursor: "pointer", background: isActive ? valColor + "22" : "transparent",
                      border: "none", borderLeft: `2px solid ${isActive ? valColor : "transparent"}`,
                      color: isActive ? valColor : "#4a6a7a", textAlign: "left",
                      display: "flex", alignItems: "center", gap: "0.5rem",
                    }}
                    onMouseOver={(e) => { e.currentTarget.style.color = valColor; e.currentTarget.style.borderLeftColor = valColor; }}
                    onMouseOut={(e)  => { if (!isActive) { e.currentTarget.style.color = "#4a6a7a"; e.currentTarget.style.borderLeftColor = "transparent"; } }}
                  >
                    <div style={{ width: 5, height: 5, borderRadius: "50%", background: valColor, flexShrink: 0 }} />
                    {val}
                  </button>
                );
              })}
  ...
  return (
    <section id="graph" className="graph-section">
      <div className="graph-header">
        <span style={{ fontFamily: "monospace", color: "#00ff88", fontSize: "0.85rem", flexShrink: 0 }}>03.</span>
        <h2 className="graph-title">Knowledge Graph</h2>
        <div className="graph-divider" />
        {!loading && (
          <span style={{ fontFamily: "monospace", fontSize: "0.65rem", color: "#4a6a7a", flexShrink: 0 }}>
            {totalNodes} nodos · {totalLinks} links
          </span>
        )}
      </div>

      <p style={{ fontFamily: "monospace", fontSize: "0.78rem", color: "#4a6a7a", marginBottom: "1.5rem" }}>
        // Mapa interactivo de conexiones entre máquinas, técnicas y herramientas
      </p>
      ...
      {!loading && graphData?.nodes?.length > 0 && (
        <>
          <div className="graph-mobile-controls">
            <button className={`graph-mobile-btn ${animating ? "active" : ""}`} onClick={runAnimation} disabled={animating}>
              {animating ? `⟳ ${animStep}...` : "▶ Animar"}
            </button>
            <button className={`graph-mobile-btn ${showLinks ? "active" : ""}`} onClick={() => setShowLinks((s) => !s)}>
              {showLinks ? "⋯ Ocultar links" : "⋯ Mostrar links"}
            </button>
            <button className={`graph-mobile-btn ${showLegend ? "active" : ""}`} onClick={() => { setShowLegend((s) => !s); setShowFilters(false); }}>
              ◉ Leyenda
            </button>
            <button className={`graph-mobile-btn ${showFilters ? "active" : ""}`} onClick={() => { setShowFilters((s) => !s); setShowLegend(false); }}>
          ...
          {showLegend && (
            <div className="graph-mobile-panel">
              <div style={{ fontFamily: "monospace", fontSize: "0.62rem", color: "#4a6a7a", letterSpacing: "2px", marginBottom: "0.6rem" }}>// Tipos</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.6rem" }}>
                {Object.entries(legendLabels).map(([type, label]) => (
                  <div key={type} style={{ display: "flex", alignItems: "center", gap: "0.4rem" }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: gColors[type] || "#4a6a7a", boxShadow: `0 0 5px ${gColors[type] || "#4a6a7a"}` }} />
                    <span style={{ fontFamily: "monospace", fontSize: "0.68rem", color: "#c8d8e8" }}>{label}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {showFilters && (
            <div className="graph-mobile-panel">
              <div style={{ fontFamily: "monospace", fontSize: "0.62rem", color: "#4a6a7a", letterSpacing: "2px", marginBottom: "0.6rem" }}>// Filtrar</div>
              <FilterList />
            </div>
          )}
        </>
      ...
      {loading && (
        <div style={{ background: "#050a0e", border: "1px solid #1a3a4a", padding: "4rem", textAlign: "center" }}>
          <p style={{ fontFamily: "monospace", color: "#00d4ff", fontSize: "0.85rem", letterSpacing: "2px" }}>
            // Cargando write-ups...
          </p>
        </div>
      )}
      ...
      {!loading && !graphData?.nodes?.length && (
        <div style={{ background: "#050a0e", border: "1px solid #1a3a4a", padding: "4rem", textAlign: "center" }}>
          <p style={{ fontFamily: "monospace", color: "#4a6a7a", fontSize: "0.85rem", letterSpacing: "2px" }}>
      ...
      {!loading && graphData?.nodes?.length > 0 && (
        <div style={{ position: "relative", background: "#050a0e", border: "1px solid #1a3a4a", overflow: "hidden" }}>
          ...
          <div className="graph-top-btns">
            <button
              onClick={runAnimation} disabled={animating}
              style={{
                fontFamily: "monospace", fontSize: "0.7rem", padding: "6px 16px",
                border: `1px solid ${animating ? "#00ff88" : "#1a3a4a"}`,
                background: animating ? "rgba(0,255,136,0.1)" : "rgba(5,10,14,0.95)",
                color: animating ? "#00ff88" : "#c8d8e8",
                cursor: animating ? "not-allowed" : "pointer", letterSpacing: "1px",
                display: "flex", alignItems: "center", gap: "0.5rem",
                backdropFilter: "blur(10px)", transition: "all 0.3s",
              }}
              onMouseOver={(e) => { if (!animating) { e.currentTarget.style.borderColor = "#00ff88"; e.currentTarget.style.color = "#00ff88"; } }}
              onMouseOut={(e)  => { if (!animating) { e.currentTarget.style.borderColor = "#1a3a4a"; e.currentTarget.style.color = "#c8d8e8"; } }}
            >
              <span style={{ fontSize: "0.8rem" }}>{animating ? "⟳" : "▶"}</span>
              {animating ? `Animando: ${animStep}...` : "Iniciar animación del grafo"}
            </button>
            <button
              ...
              style={{
                ...
                background: showLinks ? "rgba(0,212,255,0.1)" : "rgba(5,10,14,0.95)",
              ...
              onMouseOut={(e)  => { e.currentTarget.style.borderColor = showLinks ? "#00d4ff" : "#1a3a4a"; e.currentTarget.style.color = showLinks ? "#00d4ff" : "#4a6a7a"; }}
            >
          ...
          <div className="graph-legend">
            ...
            {Object.entries(legendLabels).map(([type, label]) => (
              <div key={type} style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.3rem" }}>
                <div style={{ width: 8, height: 8, borderRadius: "50%", background: gColors[type] || "#4a6a7a", boxShadow: `0 0 8px ${gColors[type] || "#4a6a7a"}` }} />
                <span style={{ fontFamily: "monospace", fontSize: "0.68rem", color: "#c8d8e8" }}>{label}</span>
              </div>
            ))}
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginTop: "0.5rem", paddingTop: "0.5rem", borderTop: "1px solid #1a3a4a" }}>
              <div style={{ width: 20, height: 0, borderTop: "1px dashed #ffffff44" }} />
              <span style={{ fontFamily: "monospace", fontSize: "0.62rem", color: "#4a6a7a" }}>Relacionadas</span>
            </div>
          </div>
          ...
          <div style={{ fontFamily: "monospace", fontSize: "0.65rem", color: "#4a6a7a", textAlign: "center", padding: "0.5rem", borderTop: "1px solid #1a3a4a" }}>
            {isMobile
              ? "pinch → zoom · drag → mover · tap → conexiones"
              : "scroll → zoom · drag → mover · click → conexiones · doble click → fijar · más conexiones = nodo más grande"}
          </div>
        </div>
      )}
      ...
      {tooltip.visible && tooltip.node && (
        <div style={{
          ...
          left: Math.min(tooltip.x + 14, (typeof window !== "undefined" ? window.innerWidth : 800) - 220),
          top: tooltip.y - 10,
          background: "rgba(10,21,32,0.97)",
          border: `1px solid ${nodeColor(tooltip.node)}44`,
          padding: "0.8rem 1rem", minWidth: "180px",
        }}>
          <div style={{ fontFamily: "monospace", fontSize: "0.62rem", color: nodeColor(tooltip.node), letterSpacing: "2px", marginBottom: "0.3rem" }}>
            {tooltip.node.type?.toUpperCase()}
          </div>
          <div style={{ fontSize: "1rem", fontWeight: 700, color: "#fff", marginBottom: "0.3rem" }}>
            {tooltip.node.label}
          </div>
          <div style={{ fontFamily: "monospace", fontSize: "0.65rem", color: "#4a6a7a", marginBottom: "0.2rem" }}>
            {tooltip.node.deg} conexiones · capa {tooltip.node.layer}
          </div>
          {tooltip.node.platform && (
            <div style={{ fontFamily: "monospace", fontSize: "0.65rem", color: "#4a6a7a", marginBottom: "0.2rem" }}>
              {tooltip.node.platform} · {tooltip.node.os} · {tooltip.node.difficulty}
            </div>
          )}
          {tooltip.node.desc && (
            <div style={{ fontFamily: "monospace", fontSize: "0.65rem", color: "#4a6a7a", marginTop: "0.3rem", lineHeight: 1.5 }}>
              {tooltip.node.desc}
            </div>
          )}
          {tooltip.node.status === "pwned" && (
            <div style={{ fontFamily: "monospace", fontSize: "0.6rem", color: "#00ff88", marginTop: "0.4rem" }}>
              ● pwned
            </div>
          )}
        </div>
