...
summary: "Lab de TryHackMe categoría RedTeam. Aplicación web PHP sobre Apache 2.4.41 / Ubuntu. Cadena de ataque completa de 6 pasos: Information Disclosure en /mail/dump.txt con credenciales en texto claro → XSS Almacenada en /contact.php sin HttpOnly → robo de cookie de agente → CSRF token predecible (MD5 del username) → escalada de rol a co-admin → SSRF via parámetro feature en dashboard accediendo a /finance.php en red interna → File Upload sin restricciones → Reverse Shell como root → Docker escape montando / del host → root en host. 3 flags obtenidas.  ---"
steps:
