...
steps:
  ...
  - id: "flag_01_post"
    ...
    content:
      - kind: "note"
        text: |
          _user_: martin@acmeitsupport.thm
          _user_: brian@acmeitsupport.thm
          _accounts_: accounts@acmeitsupport.thm
          > [!DANGER] Credenciales capturadas
          > Las credenciales almacenadas en GoPhish son el primer nivel de acceso. El siguiente paso crítico es validar estas credenciales contra sistemas reales (email legítimo, VPN, sistemas internos) para post-explotación.
          > [!TIP] Agente
          > En un engagement real, después de capturar credenciales:
          > 1. Validación inmediata — probar contra email, VPN, sistemas conocidos
          > 2. Logging — documentar quién fue vulnerable (target de re-training)
          > 3. Post-explotación — acceso a email real, calendar, contacts para inteligencia
          > 4. Lateral movement — usar creds para acceder a sistemas internos
          > 5. Persistence — crear cuenta backdoor si la misión lo requiere
          ---
  ...
  - id: "flag_02_post"
    ...
    content:
      - kind: "note"
        text: |
          _root_: Acceso a sistemas críticos internos tras escalada (AD admin, database server, etc.)
          ---
